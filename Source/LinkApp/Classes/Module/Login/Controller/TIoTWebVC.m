...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [self webViewInvokeJavaScript:@{@"name":@"pageShow"} port:@"emitEvent" withWeb:self.webView];
}
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    [self webViewInvokeJavaScript:@{@"name":@"pageHide"} port:@"emitEvent" withWeb:self.webView];
...
- (void)sendEventH5:(NSString *)eventString {
    ...
    [self webViewInvokeJavaScript:@{@"name":event} port:@"emitEvent" withWeb:self.webView];
}
...
//    message.body  --  Allowed types are NSNumber, NSString, NSDate, NSArray,NSDictionary, and NSNull.
    ...
    if (![NSString isNullOrNilWithObject:message.body]) {
        if (![NSString isNullOrNilWithObject:message.body[@"callbackId"]]) {
            ...
            [self webViewInvokeJavaScript:@{@"result":@(YES),@"callbackId":callbackIDString} port:@"callResult" withWeb:self.webView];
        }
    }
