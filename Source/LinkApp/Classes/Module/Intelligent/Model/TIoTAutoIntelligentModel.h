...
@property (nonatomic, copy) NSString *type;  //本地自行构建  添加条件 （ 0 设备状态变化 1 定时）；任务（2 设备控制，3 延时，4 选择手动，5 发送通知）

//条件
@property (nonatomic, copy) NSString *CondId; //条件ID，保证在一个联动下唯一即可
@property (nonatomic, copy) NSString *CondType;  //条件类型 0：设备属性值条件  1：定时条件
...
//任务
@property (nonatomic, copy) NSString *ActionType; // 0： 则为设备动作，具体参数设置为Data   0 ：设备动作  1：延时 2:场景 3：通知
...
@property (nonatomic, copy) NSString *Value;//比较的值
@end
