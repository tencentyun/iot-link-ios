...
//    self.searchResultTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, self.mapView.frame.size.height - 150, self.mapView.frame.size.width, self.mapView.frame.size.height) style:UITableViewStylePlain];
//    self.searchResultTableView.dataSource = self;
//    self.searchResultTableView.delegate = self;
//    [self.mapView addSubview:self.searchResultTableView];
...
//    QMSPoiData *data = _searchResultArray[indexPath.row];
    ...
    if (_searchResultArray.count != 0) {
        ...
        cell.detailTextLabel.text = [NSString stringWithFormat:@"%@%@%@%@",cellModel.ad_info.province,cellModel.ad_info.city,cellModel.ad_info.district,cellModel.address];
    }
