...
static NSString *const quality_mjpeg = @"ipc.flv?action=live-mjpg&quality=standard";
static NSString *const quality_mjpeg_audio = @"ipc.flv?action=live-audio";
...
@interface TIoTDemoPreviewMJPEGVC ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation TIoTDemoPreviewMJPEGVC
...
//    self.qualityString = quality_mjpeg_audio;
    self.qualityString = quality_mjpeg;
...
- (void)refushVideo:(NSNotification *)notify {
    ...
    if ([view isMemberOfClass:[TIoTDemoPreviewMJPEGVC class]]) {
...
//        [self.player setOptionIntValue:5000 * 1000 forKey:@"analyzeduration" ofCategory:kIJKFFOptionCategoryFormat];
        [self.player setOptionIntValue:5 * 1024 forKey:@"probesize" ofCategory:kIJKFFOptionCategoryFormat];
...
//        [self.player setOptionIntValue:SDL_FCC_RV16 forKey:@"overlay-format" ofCategory:kIJKFFOptionCategoryPlayer];
...
//        [self.player setMaxPacketNum:2];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        self.webView.frame = self.imageView.bounds;
//        [self.imageView addSubview:self.webView];
        [self.imageView insertSubview:self.webView belowSubview:self.player.view];
        [self.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:self.videoUrl]]];
        
        
//        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//            [self.webView stopLoading]; //停止播放
//            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//                [self.webView reload]; //继续播放
//            });
//        });
    });
}
