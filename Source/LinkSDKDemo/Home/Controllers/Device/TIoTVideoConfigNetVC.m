...
- (void)generateQRImage {
    [[TIoTCoreRequestObject shared] post:AppCreateDeviceBindToken Param:@{} success:^(id responseObject) {
        ...
        if (![NSObject isNullOrNilWithObject:responseObject[@"Token"]]) {
            ...
            NSString *name = self.wifiInfo[@"name"];
            NSString *bssid = self.wifiInfo[@"bssid"];
            NSDictionary *dic = @{@"bssid":bssid?:@"",
                                  @"name":name?:@"",
                                  @"pwd":self.wifiPassword.text?:@"",
                                  @"token":self.networkToken?:@"",
            };
            NSString *infoJsonString = [NSString objectToJson:dic];
            UIImage *qrImage = [TIoTCoreUtil qrCodeImageWithInfo:infoJsonString width:CGSizeMake(200, 200)];
