...
@property (nonatomic, strong) UITextField *bssid;
...
- (void)getWifiInfos
{
    ...
    else
    {
        ...
        self.bssid.placeholder = self.wifiInfo[@"bssid"];
...
- (void)setupUIViews {
    ...
    self.bssid = [[UITextField alloc]initWithFrame:CGRectMake(kLeftPadding, CGRectGetMaxY(self.wifiPassword.frame)+kInterval, kWidth, kHeight)];
    self.bssid.textColor = [UIColor colorWithHexString:kMainThemeColor];
    self.bssid.font = [UIFont systemFontOfSize:18];
    self.bssid.placeholder = @"请输入bssid";
    self.bssid.textAlignment = NSTextAlignmentCenter;
    self.bssid.returnKeyType = UIReturnKeyDone;
    self.bssid.delegate = self;
    ...
    generateQRcode.frame = CGRectMake(kLeftPadding, CGRectGetMaxY(self.token.frame)+kInterval, kWidth, kHeight);
