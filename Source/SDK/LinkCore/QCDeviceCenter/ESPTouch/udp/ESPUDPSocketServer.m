...
- (BOOL) initWithPort4: (int) port AndSocketTimeout: (int) socketTimeout
{
    ...
    if (![self setSocketTimeout:socketTimeout SocketFd:self._sck_fd4]) {
        if (DEBUG_ON) {
            DDLogInfo(@"server: sck4: setsockopt SO_RCVTIMEO fail\n");
        }
        [self close];
        return NO;
    }
...
- (Byte) receiveOneByte4
{
    ...
    else if(recNumber == 0)
    {
        if (DEBUG_ON)
        {
            DDLogInfo(@"server: receiveOneByte4 socket is closed by the other\n");
        }
    }
    else
    {
...
- (Byte) receiveOneByte6
{
    ...
    else if(recNumber == 0)
    {
        if (DEBUG_ON)
        {
            DDLogInfo(@"server: receiveOneByte6 socket is closed by the other\n");
        }
    }
    else
    {
