...
static NSString * const TIoT_ROUTER_WILDCARD_CHARACTER = @"~";
static NSString *specialCharacters = @"/?&.";

NSString *const TIoTRouterParameterURL = @"TIoTRouterParameterURL";
NSString *const TIoTRouterParameterCompletion = @"TIoTRouterParameterCompletion";
NSString *const TIoTRouterParameterUserInfo = @"TIoTRouterParameterUserInfo";

@interface TIoTRouter ()
@property (nonatomic) NSMutableDictionary *routes;
@end

@implementation TIoTRouter
+ (instancetype)sharedInstance
{
    static TIoTRouter *instance = nil;
...
+ (void)openURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo completion:(void (^)(id result))completion
{
    ...
    if (parameters) {
        TIoTRouterHandler handler = parameters[@"block"];
        if (completion) {
            parameters[TIoTRouterParameterCompletion] = completion;
        }
        if (userInfo) {
            parameters[TIoTRouterParameterUserInfo] = userInfo;
        }
        if (handler) {
...
{
    TIoTRouter *router = [TIoTRouter sharedInstance];
    
    URL = [URL stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSMutableDictionary *parameters = [router extractParametersFromURL:URL matchExactly:NO];
    TIoTRouterObjectHandler handler = parameters[@"block"];
...
- (void)addURLPattern:(NSString *)URLPattern andHandler:(TIoTRouterHandler)handler
{
...
- (void)addURLPattern:(NSString *)URLPattern andObjectHandler:(TIoTRouterObjectHandler)handler
{
    NSMutableDictionary *subRoutes = [self addURLPattern:URLPattern];
    if (handler && subRoutes) {
        subRoutes[@"_"] = [handler copy];
...
- (NSMutableDictionary *)extractParametersFromURL:(NSString *)url matchExactly:(BOOL)exactly
{
    ...
    parameters[TIoTRouterParameterURL] = url;
    ...
    for (NSString* pathComponent in pathComponents) {
        ...
        for (NSString* key in subRoutesKeys) {
            if ([key isEqualToString:pathComponent] || [key isEqualToString:TIoT_ROUTER_WILDCARD_CHARACTER]) {
                found = YES;
...
- (NSArray*)pathComponentsFromURL:(NSString*)URL
{
    ...
    if ([URL rangeOfString:@"://"].location != NSNotFound) {
        ...
        if (!URL.length) {
            [pathComponents addObject:TIoT_ROUTER_WILDCARD_CHARACTER];
        }
    }
