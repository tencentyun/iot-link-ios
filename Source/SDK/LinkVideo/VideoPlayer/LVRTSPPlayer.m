...
#import "LVUtilities.h"
#import "LVAudioStreamer.h"

@interface LVRTSPPlayer ()
@property (nonatomic, retain) LVAudioStreamer *audioController;
@end

@interface LVRTSPPlayer (private)
...
{    
    if (audioStream >= 0) {
        ...
        _audioController = [[LVAudioStreamer alloc] initWithStreamer:self];
    } else {
...
- (void)savePPMPicture:(AVPicture)pict width:(int)width height:(int)height index:(int)iFrame
{
	...
	fileName = [LVUtilities documentsPath:[NSString stringWithFormat:@"image%04d.ppm",iFrame]];
    // Open file
