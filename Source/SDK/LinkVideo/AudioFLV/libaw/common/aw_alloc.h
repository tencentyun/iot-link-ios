...
extern void aw_init_debug_alloc(void);

//停止debug alloc，调用此函数后，不再记录分配释放。所有记录清0
extern void aw_uninit_debug_alloc(void);

//返回总共alloc的size
extern size_t aw_total_alloc_size(void);

//返回总共free的size
extern size_t aw_total_free_size(void);

//打印内存alloc/free/leak状况
extern void aw_print_alloc_description(void);
