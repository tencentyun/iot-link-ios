    ...
    for (NSString *item in symbols) {
        ...
        if (result) {
            ...
            if (hexNumber >= hexStartNumber && hexNumber <= hexEndNumber) {
                ...
                NSString *addr = [NSString stringWithFormat:@"%@ 腾讯SDK内存地址范围: [%p, %p]", [formatter stringFromDate:[NSDate date]], startAddr, endAddr];
                NSString *withoutSlideAddr = [NSString stringWithFormat:@"\n%@ 腾讯SDK内存地址范围(不带偏移): [%p, %p]", [formatter stringFromDate:[NSDate date]], startAddrWithoutSlide, endAddrWithoutSlide];
...
- (instancetype)initWithAppId:(NSString *)appid secretId:(NSString *)secretId secretKey:(NSString *)secretKey projectId:(NSInteger)projectId {
    ...
    if (self) {
        ...
        NSString *addr = [NSString stringWithFormat:@"%@ 腾讯SDK内存地址范围: [%p, %p]", [formatter stringFromDate:[NSDate date]], startAddr, endAddr];
        NSLog(addr);
        NSString *withoutSlideAddr = [NSString stringWithFormat:@"%@ 腾讯SDK内存地址范围(不带偏移): [%p, %p]", [formatter stringFromDate:[NSDate date]], startAddrWithoutSlide, endAddrWithoutSlide];
        ...
        dispatch_once(&onceToken, ^{
            ...
            if (![logFileContent containsString:@"QC异常结束"] && logFileContent) {
