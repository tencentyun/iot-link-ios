...
- (void)login:(UInt32)sdkAppID user:(NSString *)userID userSig:(NSString *)userSig roomID:(NSString *)roomID {
...
- (void)call:(NSArray *)userIDs groupID:(NSString *)groupID type:(CallType)type {
    if (!self.isOnCalling) {
        ...
        self.curRoomID = self.mRoomID.intValue;//[TRTCCallingUtils generateRoomID];
